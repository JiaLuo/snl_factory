...
public class RefreshRecyclerView extends RecyclerView {
	...
	class WrapperAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
		...
		public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
			if (viewType == ITEM_TYPE_REFRESH_HEADER) {
				...
				if (viewType == headerTypes.get(mCurrentPosition - 1)) {
					mCurrentPosition++;
					return new RecyclerViewHolder(parent.getContext(), mHeaderViews.get(headerPosition++));
				}
			} else if (viewType == ITEM_TYPE_LOADMORE_FOOTER) {
	...
	/**
	 * @param noMore 设置没有更多数据了
	 */
	public void setNoMoreData(boolean noMore) {
		View footView = mFootViews.get(0);
		((LoadMoreFooterView) footView).setState(noMore ? LoadMoreFooterView.STATE_NOMORE : LoadMoreFooterView.STATE_COMPLETE);
	}
